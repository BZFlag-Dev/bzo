...
// Every host here is a reserved example name (RFC 2606) or a loopback address.
// Do not put a real deployment's hostname in a test: it runs on whatever machine
// somebody checked the repo out on, and it is testing name length, not a site.

// Hosts that fit are never altered: the platform clips them gracefully.
assert.equal(shortHostName('bzo.example.org'), 'bzo.example.org');
assert.equal(shortHostName('test-bzo.example.org'), 'test-bzo.example.org');
...
for (const host of [monster, 'bzo.example.org', 'shortlabel.a-very-long-domain-name-indeed.example.com']) {
...
assert.equal(documentTitle('bzo.example.org'), `bzo.example.org — ${PRODUCT_NAME}`);
...
assert.equal(sanitizeHost('bzo.example.org'), 'bzo.example.org');
